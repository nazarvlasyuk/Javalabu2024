...
public class Test2 {
    public static void main(String[] args) {
        // Створюємо екземпляр класу Test2
        Test2 laba = new Test2();

        // Тести для першого завдання
        System.out.println("Task 1 Results:");
        laba.printResultsTask1(2, 10);  // z = 2, k = 10
        laba.printResultsTask1(1.5, 5);  // z = 1.5, k = 5
        laba.printResultsTask1(3, 14);  // z = 3, k = 14
        laba.printResultsTask1(3, 15);  // Неправильний аргумент для k

        // Тести для другого завдання
        System.out.println("\nTask 2 Results:");
        laba.printResultsTask2(10, 2);  // t = 10, l = 2 (парне)
        laba.printResultsTask2(10, 3);  // t = 10, l = 3 (непарне)
        laba.printResultsTask2(5, 6);  // t = 5, l = 6 (парне)

        // Тести для третього завдання
        System.out.println("\nTask 3 Results:");
        laba.printResultsTask3(0.001);  // epsilon = 0.001
        laba.printResultsTask3(0.0001);  // epsilon = 0.0001
        laba.printResultsTask3(-0.01);  // Неправильний аргумент для epsilon
    }

    // Завдання 1: Метод для обчислення суми ряду
    public double calculateSumTask1(double z, int k) {
        if (k >= 15) {
    ...
    public void printResultsTask1(double z, int k) {
        try {
    ...
    public int calculateFunctionTask2(double t, int l) {
        if (l % 2 == 0) {
    ...
    public void printResultsTask2(double t, int l) {
        try {
    ...
    public double calculateInfiniteSumTask3(double epsilon) {
        if (epsilon <= 0) {
    ...
    public void printResultsTask3(double epsilon) {
        try {
